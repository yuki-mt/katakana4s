...
object EnglishVowel{
  ...
  def convert(consonant: Option[Alphabet], vowel: Alphabet, nexts:(Option[Sound], Option[Sound]), size: Int): Katakana = {
    if(vowel == "e" && nexts._1.isEmpty && size > 1){
      ...
      if(vowel == "r") "アー"
        ...
        val subKatakana =
          if (nexts._2.isEmpty && nexts._1.exists(n => n.consonant.exists(c => c.size > 1 && c.distinct.size == 1) && n.vowel.exists(v => Set("e", "or", "er", "el", "le") contains v))) {
  ...
  def getDefaultVowel(consonant: Alphabet) = {
    if(consonant.endsWith("t") || consonant.endsWith("d")) "オ"
    else if(consonant == "ch") "イ"
    else "ウ"
  }
}
